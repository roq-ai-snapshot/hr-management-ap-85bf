...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.applicant?.length > 0) {
      const create_applicant = body.applicant;
      body.applicant = {
    ...
    if (body?.hr_assistant?.length > 0) {
      const create_hr_assistant = body.hr_assistant;
      body.hr_assistant = {
        create: create_hr_assistant,
      };
    } else {
      delete body.hr_assistant;
    }
