...
import { getHrAssistants } from 'apiSdk/hr-assistants';
...
function DashboardPage() {
  ...
  const hrAssistantStat = useSWR<PaginatedInterface<HrAssistantInterface>>(
    () => `/hr-assistants`,
    () => getHrAssistants({ limit: 1, offset: 0 }),
  );
  ...
  const applicantStat = useSWR<PaginatedInterface<ApplicantInterface>>(
    () => `/applicants`,
    () => getApplicants({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Hr Assistants',
      result: hrAssistantStat?.data?.totalCount,
      isLoading: hrAssistantStat.isLoading,
    ...
    {
      title: 'Applicants',
      result: applicantStat?.data?.totalCount,
