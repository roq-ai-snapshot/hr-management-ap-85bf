...
import { getHrAssistants, deleteHrAssistantById } from 'apiSdk/hr-assistants';
import { HrAssistantInterface } from 'interfaces/hr-assistant';

type ColumnType = ColumnDef<HrAssistantInterface, unknown>;

interface HrAssistantListPageProps {
...
export function HrAssistantListPage(props: HrAssistantListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getHrAssistants({
        ...
        order: params.order,
        searchTermKeys: ['assigned_tasks.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<HrAssistantInterface>>(
    () => `/hr-assistants?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteHrAssistantById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: HrAssistantInterface) => {
    if (hasAccess('hr_assistant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/hr-assistants/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'assigned_tasks', header: 'Assigned Tasks', accessorKey: 'assigned_tasks' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/hr-assistants/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('hr_assistant', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/hr-assistants/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('hr_assistant', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="hr_assistant" />
        </Flex>

        {hasAccess('hr_assistant', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/hr-assistants/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
